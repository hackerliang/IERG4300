...
mnist['train_images'] = pca.fit_transform(mnist['train_images'])
mnist['train_images'] = mnist['train_images'].reshape((60000, 25))
mnist['train_labels'] = mnist['train_labels'].reshape((60000, 1))
train_image_label = np.concatenate((mnist['train_images'], mnist['train_labels']), axis=1)
...
label_cluster_id = pd.DataFrame(data={
    'ground_truth_label': train_image_label_cluster[:, [25]].flatten(),
    'assigned_cluster': train_image_label_cluster[:, [26]].flatten()
}).astype(int)
